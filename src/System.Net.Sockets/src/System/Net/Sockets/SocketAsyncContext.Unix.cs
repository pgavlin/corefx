...
namespace System.Net.Sockets
{
    ...
    {
        private sealed class NonBlockingIOContext
        {
            ...
            private static NonBlockingIOContext s_closedContext;
            public static NonBlockingIOContext ClosedContext
            {
                get
                {
                    if (Volatile.Read(ref s_closedContext) == null)
                    {
                        var ctx = new NonBlockingIOContext(new SocketAsyncContext(-1), null);
                        ctx.Close();

                        Volatile.Write(ref s_closedContext, ctx);
                    }

                    return s_closedContext;
                }
            }
            ...
            public NonBlockingIOContext(SocketAsyncContext asyncContext, SocketAsyncEngine engine)
            {
            ...
            private void Register(Interop.Sys.SocketEvents events)
            {
                ...
                if (!_engine.TryRegister(_asyncContext._fileDescriptor, _registeredEvents, events, _handle, out errorCode))
                {
                    if (_registeredEvents == Interop.Sys.SocketEvents.None)
                    {
                        _handle.Free();
            ...
            private void UnregisterRead()
            {
                ...
                {
                    Interop.Error errorCode;
                    bool unregistered = _engine.TryRegister(_asyncContext._fileDescriptor, _registeredEvents, events, _handle, out errorCode);
                    if (unregistered)
                    {
                        _registeredEvents = events;
                    }
            ...
            private void Unregister()
            {
                ...
                Interop.Error errorCode;
                bool unregistered = _engine.TryRegister(_asyncContext._fileDescriptor, _registeredEvents, Interop.Sys.SocketEvents.None, _handle, out errorCode);
            ...
            public SocketError Accept(byte[] socketAddress, ref int socketAddressLen, out int acceptedFd)
            {
                Debug.Assert(socketAddress != null);
                Debug.Assert(socketAddressLen > 0);

                SocketError errorCode;
                if (SocketPal.TryCompleteAccept(_asyncContext._fileDescriptor, socketAddress, ref socketAddressLen, out acceptedFd, out errorCode))
                {
                    Debug.Assert(errorCode == SocketError.Success || acceptedFd == -1);
                    return errorCode;
                }

                if (!_asyncContext._isManagedSocketBlocking)
                {
                ...
                using (var @event = new ManualResetEventSlim(false, 0))
                {
                    ...
                    while (!TryBeginOperation(ref _acceptOrConnectQueue, operation, Interop.Sys.SocketEvents.Read, out isStopped))
                    {
                        ...
                        if (operation.TryComplete(_asyncContext._fileDescriptor))
                        {
            ...
            public SocketError Connect(byte[] socketAddress, int socketAddressLen)
            {
                ...
                SocketError errorCode;
                if (SocketPal.TryStartConnect(_asyncContext._fileDescriptor, socketAddress, socketAddressLen, out errorCode))
                {
                    return errorCode;
                }

                if (!IsManagedSocketBlocking)
                {
                    return SocketError.WouldBlock;
                }
            ...
            public SocketError Receive(byte[] buffer, int offset, int count, ref SocketFlags flags, out int bytesReceived)
            {
                int socketAddressLen = 0;
                return ReceiveFrom(buffer, offset, count, ref flags, null, ref socketAddressLen, out bytesReceived);
            }
            ...
            public SocketError ReceiveFrom(byte[] buffer, int offset, int count, ref SocketFlags flags, byte[] socketAddress, ref int socketAddressLen, out int bytesReceived)
            {
                SocketFlags receivedFlags;
                SocketError errorCode;
                if (SocketPal.TryCompleteReceiveFrom(_asyncContext._fileDescriptor, buffer, offset, count, flags, socketAddress, ref socketAddressLen, out bytesReceived, out receivedFlags, out errorCode))
                {
                    flags = receivedFlags;
                    return errorCode;
                }
                ...
                using (var @event = new ManualResetEventSlim(false, 0))
                {
                    ...
                    bool signaled = operation.Wait(_asyncContext._receiveTimeout);
                    socketAddressLen = operation.SocketAddressLen;
                    flags = operation.ReceivedFlags;
                    bytesReceived = operation.BytesTransferred;
            ...
            public SocketError Receive(IList<ArraySegment<byte>> buffers, ref SocketFlags flags, out int bytesReceived)
            {
                return ReceiveFrom(buffers, ref flags, null, 0, out bytesReceived);
            }
            ...
            public SocketError ReceiveFrom(IList<ArraySegment<byte>> buffers, ref SocketFlags flags, byte[] socketAddress, int socketAddressLen, int out int bytesReceived)
            {
                SocketFlags receivedFlags;
                SocketError errorCode;
                if (SocketPal.TryCompleteReceiveFrom(_asyncContext._fileDescriptor, buffers, flags, socketAddress, ref socketAddressLen, out bytesReceived, out receivedFlags, out errorCode))
                {
                    flags = receivedFlags;
                    return errorCode;
                }
            ...
            public SocketError ReceiveMessageFrom(byte[] buffer, int offset, int count, ref SocketFlags flags, byte[] socketAddress, ref int socketAddressLen, bool isIPv4, bool isIPv6, out IPPacketInformation ipPacketInformation, out int bytesReceived)
            {
                SocketFlags receivedFlags;
                SocketError errorCode;
                if (SocketPal.TryCompleteReceiveMessageFrom(_asyncContext._fileDescriptor, buffer, offset, count, flags, socketAddress, ref socketAddressLen, isIPv4, isIPv6, out bytesReceived, out receivedFlags, out ipPacketInformation, out errorCode))
                {
                    flags = receivedFlags;
                    return errorCode;
                }
            ...
            public SocketError Send(byte[] buffer, int offset, int count, SocketFlags flags, out int bytesSent)
            {
                return SendTo(buffer, offset, count, flags, null, 0, out bytesSent);
            }
            ...
            public SocketError SendTo(byte[] buffer, int offset, int count, SocketFlags flags, byte[] socketAddress, int socketAddressLen, out int bytesSent)
            {
                bytesSent = 0;
                SocketError errorCode;
                if (SocketPal.TryCompleteSendTo(_asyncContext._fileDescriptor, buffer, ref offset, ref count, flags, socketAddress, socketAddressLen, ref bytesSent, out errorCode))
                {
                    return errorCode;
                }
                ...
                using (var @event = new ManualResetEventSlim(false, 0))
                {
                    ...
                    bool signaled = operation.Wait(_asyncContext._sendTimeout);
                    bytesSent = operation.BytesTransferred;
                    return signaled ? operation.ErrorCode : SocketError.TimedOut;
                }
            }
            ...
            public SocketError Send(IList<ArraySegment<byte>> buffers, SocketFlags flags, out int bytesSent)
            {
                return SendTo(buffers, flags, null, 0, out bytesSent);
            }
            ...
            public SocketError SendTo(IList<ArraySegment<byte>> buffers, SocketFlags flags, byte[] socketAddress, int socketAddressLen, out int bytesSent)
            {
                bytesSent = 0;
                int bufferIndex = 0;
                int offset = 0;
                SocketError errorCode;
                if (SocketPal.TryCompleteSendTo(_asyncContext._fileDescriptor, buffers, ref bufferIndex, ref offset, flags, socketAddress, socketAddressLen, ref bytesSent, out errorCode))
                {
                    return errorCode;
                }
            ...
            public unsafe void HandleEvents(Interop.Sys.SocketEvents events)
            {
                ...
                lock (_closeLock)
                {
                    ...
                    if ((events & Interop.Sys.SocketEvents.ReadClose) != 0)
                    {
                        ...
                        {
                            TransferOperation op = receiveQueue.Head;
                            bool completed = op.TryCompleteAsync(_asyncContext._fileDescriptor);
                            Debug.Assert(completed);
                            receiveQueue.Dequeue();
                        }
                    ...
                    if ((events & Interop.Sys.SocketEvents.Read) != 0)
                    {
                        ...
                        if (acceptTail != null)
                        {
                            ...
                            do
                            {
                                ...
                                if (!op.TryCompleteAsync(_asyncContext._fileDescriptor))
                                {
                                    break;
                                }
        ...
        private enum UnderlyingIOKind : int
        {
            Undetermined = 0,
            InTransition = 1,
            Blocking = 2,
        ...
        private bool _isManagedSocketBlocking = true;
        private NonBlockingIOContext _nonblockingIOContext;
        private int _underlyingIOKind; // Actually of type UnderlyingIOKind

        public int ReceiveTimeout
        {
            get
            {
                if (UsesBlockingIO(null))
                {
                    // TODO: Get the underlying socket option
                }
            ...
            set
            {
                ...
                if (UsesBlockingIO(null))
                {
                    // TODO: Set the underlying socket option
                }
        ...
        public bool IsManagedSocketBlocking
        {
            get
            {
                if (UsesBlockingIO(null))
                ...
                return _isManagedSocketBlocking;
            }
            set
            {
                if (UsesBlockingIO(null))
                ...
                _isManagedSocketBlocking = value;
            }
        }

        private bool UsesBlockingIO(bool? isBlockingCall)
        {
            UnderlyingIOKind kind = (UnderlyingIOKind)Volatile.Read(ref _underlyingIOKind);
            if (kind == UnderlyingIOKind.Undetermined && isBlockingCall != null)
            {
                kind = (UnderlyingIOKind)Interlocked.CompareExchange(ref _underlyingIOKind, (int)UnderlyingIOKind.InTransition, (int)UnderlyingIOKind.Undetermined);
                if (kind == UnderlyingIOKind.Undetermined)
                {
                    if ((bool)isBlockingCall)
                    {
                        // TODO: Set timeouts and blocking mode
                        Volatile.Write(ref _underlyingIOKind, (int)UnderlyingIOKind.Blocking);
                        return false;
                    }
                    else
                    {
                        // TODO: Make underlying socket non-blocking
                        _nonblockingIOContext = new NonBlockingIOContext(this, SocketAsyncEngine.Instance);
                        Volatile.Write(ref _underlyingIOKind, (int)UnderlyingIOKind.NonBlocking);
                        return true;
                    }
                }
            }

            if (kind == UnderlyingIOKind.InTransition)
            {
                var spinWait = new SpinWait();
                do
                {
                    spinWait.SpinOnce();
                    kind = (UnderlyingIOKind)Volatile.Read(ref _underlyingIOKind);
                } while (kind == UnderlyingIOKind.InTransition);
            }

            Debug.Assert(kind == UnderlyingIOKind.Blocking || kind == UnderlyingIOKind.NonBlocking || isBlockingCall == null);
            return kind == UnderlyingIOKind.Blocking;
        }

        public SocketError Accept(byte[] socketAddress, ref int socketAddressLen, out int acceptedFd)
        {
            if (UsesBlockingIO(true))
            {
                // TODO: Call directly to accept
            }

            return _nonblockingIOContext.Accept(socketAddress, ref socketAddressLen, out acceptedFd);
        }
